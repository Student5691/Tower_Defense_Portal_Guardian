...
class Turret(pg.sprite.Sprite):
    ...
    def update(self, enemy_group, _world): #cooldown manager for next round to be fired
        if pg.time.get_ticks() - self.last_shot > self.cooldown / _world.game_speed:
            self.targeting(enemy_group)
    ...
    def targeting(self, enemy_group):
        x_dist = 0
        y_dist = 0
        targets = []
        target_ids = []
    ...
    def upgrade(self, world):
        ...
        if self.upgrade_level < self.upgrade_limit:
            ...
            self.range_rect = self.range_image.get_rect()
            self.range_rect.center = self.rect.center
            world.undo_deck.append(partial(self.undo, world)) #deque append

    def undo(self, world): #this function gets appended to the deque and called in a LIFO order
